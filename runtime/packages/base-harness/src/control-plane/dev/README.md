This is a plugin to simulate a remote environment locally. Add this to `.base-harness/base-harness.jsonc`:

```json
  "plugin": ["../packages/base-harness/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Base Harness server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/base-harness/script/run-workspace-server
```

With the plugin install, you can now run Base Harness and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
