...
export async function checkPluginCompatibility(target: string, baseHarnessVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(baseHarnessVersion) || semver.major(baseHarnessVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["base-harness"]
  if (typeof range !== "string") return
  if (!semver.satisfies(baseHarnessVersion, range)) {
    throw new Error(`Plugin requires base-harness ${range} but running ${baseHarnessVersion}`)
  }
}
