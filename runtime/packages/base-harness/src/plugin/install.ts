...
export type PatchDeps = {
  ...
  files: (dir: string, name: "base-harness" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".base-harness")
}

function patchName(kind: Kind): "base-harness" | "tui" {
  if (kind === "server") return "base-harness"
  return "tui"
}
