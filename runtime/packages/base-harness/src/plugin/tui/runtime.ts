...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@base-harness/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@base-harness/core/effect/app-node-builder"
import { errorData, errorMessage } from "@base-harness/tui/util/error"
...
import { hasTheme, upsertTheme } from "@base-harness/tui/context/theme"
import { Global } from "@base-harness/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@base-harness/core/util/flock"
import { Flag } from "@base-harness/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@base-harness/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@base-harness/core/v1/config/plugin"
import { createCommandShim } from "@base-harness/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@base-harness/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".base-harness"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".base-harness", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".base-harness", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.BASE_HARNESS_PURE ? [] : pluginOrigins
    if (Flag.BASE_HARNESS_PURE && pluginOrigins.length) {
    }
