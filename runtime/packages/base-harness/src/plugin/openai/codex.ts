...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "base-harness"
      output.headers["User-Agent"] = `base-harness/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
