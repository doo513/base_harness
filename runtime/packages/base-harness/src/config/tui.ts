...
import { isRecord } from "@base-harness/tui/util/record"
...
import { TuiKeybind } from "@base-harness/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@base-harness/core/installation/version"
...
import { TuiConfig } from "@base-harness/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@base-harness/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old base-harness.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.base-harness`
  // folders between cwd and home, and BASE_HARNESS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.BASE_HARNESS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit BASE_HARNESS_TUI_CONFIG override, if set.
  if (Flag.BASE_HARNESS_TUI_CONFIG) {
    const configFile = Flag.BASE_HARNESS_TUI_CONFIG
  ...
  // 4. `.base-harness` directories (and BASE_HARNESS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".base-harness") || dir === Flag.BASE_HARNESS_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".base-harness") && dir !== Flag.BASE_HARNESS_CONFIG_DIR) continue
  ...
  Effect.gen(function* () {
    ...
    const deps = yield* Effect.forEach(
      ...
      (dir) =>
        npm
          .install(dir)
          .pipe(Effect.andThen(Effect.promise(() => provisionBuiltinPluginModule(dir))))
