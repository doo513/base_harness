...
const TUI_SCHEMA_URL = "https://base-harness.local/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from base-harness.jsonc files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const configs = await baseHarnessFiles(input)
...
async function baseHarnessFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    path.join(Global.Path.config, "base-harness.jsonc"),
    ...(await Filesystem.findUp(["base-harness.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(path.join(dir, "base-harness.jsonc"))
  }
  if (Flag.BASE_HARNESS_CONFIG) files.push(Flag.BASE_HARNESS_CONFIG)
