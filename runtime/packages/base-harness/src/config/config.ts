...
import { RemoteAuthError } from "@base-harness/core/v1/config/error"
import { ConfigPermissionV1 } from "@base-harness/core/v1/config/permission"
...
function globalConfigFile() {
  return path.join(Global.Path.config, "base-harness.jsonc")
}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://base-harness.local/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.BASE_HARNESS_CONFIG && !Flag.BASE_HARNESS_CONFIG_DIR && !Flag.BASE_HARNESS_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://base-harness.local/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "base-harness.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "BASE_HARNESS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://base-harness.local/config.json"
        ...
        if (Flag.BASE_HARNESS_CONFIG) {
          yield* merge(Flag.BASE_HARNESS_CONFIG, yield* loadFile(Flag.BASE_HARNESS_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.BASE_HARNESS_CONFIG })
        }

        if (!Flag.BASE_HARNESS_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("base-harness", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.BASE_HARNESS_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from BASE_HARNESS_CONFIG_DIR", { path: Flag.BASE_HARNESS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".base-harness") || dir === Flag.BASE_HARNESS_CONFIG_DIR) {
            for (const file of ["base-harness.jsonc"]) {
          ...
          const dep = yield* npmSvc
            .install(dir)
            .pipe(
              Effect.andThen(Effect.promise(() => provisionBuiltinPluginModule(dir))),
              Effect.exit,
              Effect.tap((exit) =>
                Exit.isFailure(exit)
          ...
          // Auto-discovered plugins under `.base-harness/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.BASE_HARNESS_CONFIG_CONTENT) {
          const source = "BASE_HARNESS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.BASE_HARNESS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from BASE_HARNESS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["BASE_HARNESS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("BASE_HARNESS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.BASE_HARNESS_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.BASE_HARNESS_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("BASE_HARNESS_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.BASE_HARNESS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.BASE_HARNESS_DISABLE_PRUNE) {
