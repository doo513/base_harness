...
export class Service extends Context.Service<Service, Interface>()("@base-harness/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
      authMethod._meta = {
        "terminal-auth": {
          command: "base-harness",
          args: ["auth", "login"],
          label: "Base Harness Login",
        },
      }
    }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Base Harness prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Base Harness service failure", service })
}
