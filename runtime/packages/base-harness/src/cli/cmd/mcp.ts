...
import { ConfigMCPV1 } from "@base-harness/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: base-harness mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in base-harness.jsonc:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  const candidates = [path.join(baseDir, "base-harness.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".base-harness", "base-harness.jsonc"))
  }
  ...
  // JSONC is the only supported user and project configuration format.
  return candidates[0]
}
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "base-harness-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
