...
Practical reference for the current tool-migration state in `packages/base-harness`.
...
Tool tests should use the existing Effect helpers in `packages/base-harness/test/lib/effect.ts`:
