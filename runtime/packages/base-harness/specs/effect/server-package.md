...
- The server still lives in `packages/base-harness`.
...
Until enough shared service code lives outside `packages/base-harness`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/base-harness`

It should not import `packages/base-harness` services while `packages/base-harness`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/base-harness` implementation details.
