...
- Author package entrypoint is `@base-harness/plugin/tui`.
...
```json
{
  "$schema": "https://base-harness.local/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@base-harness/plugin/tui`.
- `@base-harness/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@base-harness/plugin/tui"
...
- The value is a semver range checked against the running Base Harness version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.base-harness` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.base-harness` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.base-harness`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.base-harness`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.base-harness` dir or `BASE_HARNESS_CONFIG_DIR`, Base Harness installs `@base-harness/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@base-harness/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Base Harness's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Base Harness registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Base Harness pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@base-harness/plugin/tui`.
...
- `message` is required; `title` defaults to `"base-harness"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Local plugins persist installed themes under the local `.base-harness/themes` area near the plugin config source.
...
- `--pure` / `BASE_HARNESS_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.base-harness/plugins/tui-smoke.tsx`
- Local vim plugin: `.base-harness/plugins/tui-vim.tsx`
- Local smoke config: `.base-harness/tui.json`
- Local smoke theme: `.base-harness/plugins/smoke-theme.json`
