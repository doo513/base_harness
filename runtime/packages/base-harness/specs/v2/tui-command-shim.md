...
Delete `packages/base-harness/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/base-harness/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/base-harness/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/base-harness`
- TUI plugin loader tests in `packages/base-harness` if runtime plugin API wiring changed
