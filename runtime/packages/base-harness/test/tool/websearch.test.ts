...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.BASE_HARNESS_WEBSEARCH_PROVIDER

    try {
      process.env.BASE_HARNESS_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.BASE_HARNESS_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.BASE_HARNESS_WEBSEARCH_PROVIDER
      else process.env.BASE_HARNESS_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is enabled for Base Harness providers or explicit websearch provider flags", () => {
