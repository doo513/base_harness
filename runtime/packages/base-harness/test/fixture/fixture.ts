...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  ...
  if (options?.git) {
    ...
    await $`git config user.email "test@base-harness.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "base-harness.jsonc"),
      JSON.stringify({
...
export function tmpdirScoped<E = never, R = never>(options?: {
  ...
  return Effect.gen(function* () {
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@base-harness.test")
    ...
    if (options?.config) {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://base-harness.local/config.json", ...resolved }),
        ),
      )
    }
