...
import { EventSequenceTable } from "@base-harness/core/event/sql"
...
import { WorkspaceV2 } from "@base-harness/core/workspace"
import { WorkspaceTable } from "@base-harness/core/control-plane/workspace.sql"
...
const originalEnv = {
  BASE_HARNESS_AUTH_CONTENT: process.env.BASE_HARNESS_AUTH_CONTENT,
  BASE_HARNESS_EXPERIMENTAL_WORKSPACES: process.env.BASE_HARNESS_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.BASE_HARNESS_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.BASE_HARNESS_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.BASE_HARNESS_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.BASE_HARNESS_EXPERIMENTAL_WORKSPACES).toBe("true")
