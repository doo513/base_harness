...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-base-harness-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-base-harness-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://base-harness.local/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-base-harness-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-base-harness-workspace")
