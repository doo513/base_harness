...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.BASE_HARNESS_EXPERIMENTAL_DISABLE_FILEWATCHER
      process.env.BASE_HARNESS_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
      return previous
    }),
    () => effect,
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.BASE_HARNESS_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.BASE_HARNESS_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
