...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      BASE_HARNESS_EXPERIMENTAL_WORKSPACES: Flag.BASE_HARNESS_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.BASE_HARNESS_EXPERIMENTAL_WORKSPACES = original.BASE_HARNESS_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
