...
const original = {
  ...
  envPassword: process.env.BASE_HARNESS_SERVER_PASSWORD,
  envUsername: process.env.BASE_HARNESS_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.BASE_HARNESS_SERVER_PASSWORD
  else process.env.BASE_HARNESS_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.BASE_HARNESS_SERVER_USERNAME
  else process.env.BASE_HARNESS_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.BASE_HARNESS_SERVER_PASSWORD = auth.password
  Flag.BASE_HARNESS_SERVER_USERNAME = auth.username
  process.env.BASE_HARNESS_SERVER_PASSWORD = auth.password
  process.env.BASE_HARNESS_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-base-harness-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-base-harness-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-base-harness-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.BASE_HARNESS_DISABLE_DEFAULT_PLUGINS
    process.env.BASE_HARNESS_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.BASE_HARNESS_DISABLE_DEFAULT_PLUGINS
      else process.env.BASE_HARNESS_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-base-harness-ticket": "1" },
      })
