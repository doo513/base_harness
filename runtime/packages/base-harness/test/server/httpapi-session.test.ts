...
import { Ripgrep } from "@base-harness/core/ripgrep"
...
import { SessionInputTable, SessionMessageTable, SessionTable } from "@base-harness/core/session/sql"
...
const originalWorkspaces = Flag.BASE_HARNESS_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.BASE_HARNESS_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-base-harness-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-base-harness-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-base-harness-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.BASE_HARNESS_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-base-harness-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "base-harness", "src")
        ...
        const query = new URLSearchParams({
          scope: "project",
          path: "packages/base-harness/src",
          directory: currentDir,
        })
        const headers = { "x-base-harness-directory": test.directory }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-base-harness-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-base-harness-directory": globalWorktreeSentinel, "content-type": "application/json" }
