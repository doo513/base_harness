...
import { HttpRecorder } from "@base-harness/http-recorder"
import { HttpRecorderInternal } from "@base-harness/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@base-harness/llm"
import { RequestExecutor } from "@base-harness/llm/route"
...
const zenURL = (connection: string) => `https://console.base-harness.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.BASE_HARNESS_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("BASE_HARNESS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("BASE_HARNESS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  {
    id: "opencode-proxy",
    name: "Base Harness proxy",
    providerID: ProviderV2.ID.opencode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["opencode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.BASE_HARNESS_RECORD_CONSOLE_TOKEN && process.env.BASE_HARNESS_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "Base Harness Zen",
        env: ["BASE_HARNESS_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.BASE_HARNESS_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.BASE_HARNESS_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.BASE_HARNESS_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BASE_HARNESS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("BASE_HARNESS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("BASE_HARNESS_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set BASE_HARNESS_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "base-harness.jsonc"),
      JSON.stringify({ $schema: "https://base-harness.local/config.json", ...scenario.config(model) }),
    ),
  )
