...
import { SessionMessageTable } from "@base-harness/core/session/sql"
...
import { Shell } from "@base-harness/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@base-harness/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://base-harness.local/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/base-harness/src/session/processor.ts")
