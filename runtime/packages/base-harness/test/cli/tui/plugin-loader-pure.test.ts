...
`,
  ...
  const pure = process.env.BASE_HARNESS_PURE
  const meta = process.env.BASE_HARNESS_PLUGIN_META_FILE
  process.env.BASE_HARNESS_PURE = "1"
  process.env.BASE_HARNESS_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.BASE_HARNESS_PURE
    } else {
      process.env.BASE_HARNESS_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.BASE_HARNESS_PLUGIN_META_FILE
    } else {
      process.env.BASE_HARNESS_PLUGIN_META_FILE = meta
    }
  }
})
