  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Base Harness")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.BASE_HARNESS_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
