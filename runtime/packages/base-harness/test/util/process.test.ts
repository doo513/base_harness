...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.BASE_HARNESS_TEST ?? "")'), {
      env: {
        BASE_HARNESS_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "BASE_HARNESS_TEST_SHELL"], {
      shell: true,
      env: {
        BASE_HARNESS_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("BASE_HARNESS_TEST_SHELL=ok")
  })
