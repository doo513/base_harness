...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "base-harness.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "base-harness.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "base-harness.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "base-harness.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["base-harness.jsonc", "base-harness.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(tmp.path, "base-harness.jsonc"),
        path.join(project, "base-harness.jsonc"),
        path.join(project, "base-harness.jsonc"),
      ])
    })
  })
