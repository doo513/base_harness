...
export class WorkspaceCandidateStore {
  // Snapshot reads and publication share this queue; worker execution does not.
  ...
  private readonly writes = new Map<RootState, Promise<void>>()
  private readonly persistenceErrors = new Map<RootState, OrchestrationError>()
  private generation = 0

  constructor(private readonly snapshotWriter: typeof writeAtomicSnapshot = writeAtomicSnapshot) {}

  assertPersistenceHealthy(root: RootState) {
    const error = this.persistenceErrors.get(root)
    if (error) throw error
  }

  enqueuePersistence(root: RootState, body: string) {
    const directory = root.stateDirectory
    const runId = root.runId
    const generation = this.generation
    const current = () => generation === this.generation && root.runId === runId
    const preceding = this.writes.get(root) ?? Promise.resolve()
    let tracked: Promise<void>
    tracked = preceding.then(async () => {
      if (!current() || this.persistenceErrors.has(root)) return
      await this.snapshotWriter(path.join(directory, "orchestration.json"), body, { isCurrent: current })
    }).catch(error => {
      if (current()) this.persistenceErrors.set(root, new OrchestrationError(
        "PERSISTENCE_ERROR", "Workspace state persistence failed: " + (error instanceof Error ? error.message : String(error)),
      ))
    }).finally(() => {
      if (this.writes.get(root) === tracked) this.writes.delete(root)
    })
    this.writes.set(root, tracked)
  }

  async flushPersistence(root?: RootState): Promise<void> {
    while (true) {
      const pending = root
        ? [this.writes.get(root)].filter((value): value is Promise<void> => value !== undefined)
        : [...this.writes.values()]
      if (!pending.length) break
      await Promise.all(pending)
    }
    if (root) this.assertPersistenceHealthy(root)
    else if (this.persistenceErrors.size) throw this.persistenceErrors.values().next().value
  }
...
const storeContext = new AsyncLocalStorage<WorkspaceCandidateStore>()
let activeStore: WorkspaceCandidateStore | undefined

export function withWorkspaceCandidateStore<T>(store: WorkspaceCandidateStore, action: () => T): T {
  return storeContext.run(store, action)
}
...
const currentStore = () => storeContext.getStore() ?? (activeStore ??= new WorkspaceCandidateStore())
...
function snapshotValue(root: RootState) {
  return {
    ...
    goal: root.goal,
    phase: root.phase,
    explorationScopeId: root.explorationScopeId,
    ...
    candidates: [...candidates.values()]
      .filter((candidate) => candidate.runId === root.runId)
...
function persist(root: RootState) {
  currentStore().enqueuePersistence(root, JSON.stringify(snapshotValue(root), null, 2))
}

export async function flushPersistence(sessionID?: string): Promise<void> {
  const root = sessionID === undefined ? undefined : rootFor(sessionID)
...
function instruction(root: RootState) {
  if (root.phase === "integration") {
    return "Continue root integration under the accepted GoalContract and WorkGraph. Preserve committed workers; do not recreate the contract or plan."
  }
  if (root.phase === "repair") {
    return "Apply only the current verifier-scoped repair under the existing contract and plan. Preserve unrelated completed work."
  }
...
export function beginPrompt(input: {
  ...
  if (child && child.kind !== "root") {
    if (child.kind === "meta_review") {
      return { explore: false, instruction: "Review only the supplied contract or plan. Read-only workspace inspection is allowed; do not implement work or issue Evidence or Ready." }
    }
    if (child.kind === "exploration") {
      return { explore: false, instruction: "Inspect the workspace read-only and return the bounded exploration report." }
    }
...
export function setRunIdentity(sessionID: string, runId: string) {
  const root = rootFor(sessionID)
  if (!root) throw new OrchestrationError("PHASE_VIOLATION", "The root scope is not open")
  root.runId = runId
  root.stateDirectory = path.join(stateBase(), "runs", safeID(runId), "orchestration")
  persist(root)
}

export function beginPlanning(sessionID: string) {
  const root = rootFor(sessionID)
  if (!root || !root.contractClaimIds.size) {
    throw new OrchestrationError("PHASE_VIOLATION", "Planning requires an accepted contract")
  }
  if (root.active.size || root.graph || !["direct", "exploration", "planning"].includes(root.phase)) {
    throw new OrchestrationError("PHASE_VIOLATION", "Planning cannot replace active or terminal work")
  }
  root.phase = "planning"
  persist(root)
}

export function beginDirectExecution(sessionID: string) {
  const root = rootFor(sessionID)
  if (!root || !root.contractClaimIds.size) {
    throw new OrchestrationError("PHASE_VIOLATION", "Direct execution requires an accepted contract")
  }
  if (root.active.size || root.graph) {
    throw new OrchestrationError("PHASE_VIOLATION", "A WorkGraph cannot be replaced by direct execution")
  }
  root.phase = "direct"
  persist(root)
}
...
export function startChild(input: {
  ...
  if (input.subagentType === "meta-review") {
    if (input.parentSessionID !== root.sessionID || input.workUnitId || prior
        || !["direct", "exploration", "planning"].includes(root.phase)) {
      throw new OrchestrationError("PHASE_VIOLATION", "Meta review requires a fresh read-only child of the planning root")
    }
    const scope: ScopeState = {
      sessionID: input.sessionID, rootSessionID: root.sessionID,
      kind: "meta_review", status: "running", candidateRevision: 0,
  ...
  if (input.subagentType === "explore") {
    ...
    if (input.parentSessionID !== root.sessionID || input.workUnitId) {
      throw new OrchestrationError("PHASE_VIOLATION", "Exploration requires an unassigned child of the root")
    }
    if (root.explorationScopeId) {
      if (root.explorationScopeId === input.sessionID && root.phase === "exploration"
          && prior?.rootSessionID === root.sessionID && prior.kind === "exploration"
          && prior.status === "running") return assignment(prior, root)
      throw new OrchestrationError("PHASE_VIOLATION", "Exploration is allowed exactly once before planning")
    }
    const explicitDiscovery = root.phase === "direct"
      && !root.contractClaimIds.size && !root.contractCriterionIds.size
    if (prior || root.active.size || root.graph || (root.phase !== "exploration" && !explicitDiscovery)) {
      throw new OrchestrationError("PHASE_VIOLATION", "Exploration requires an idle discovery root before planning")
    }
    // A typed explicit request can start discovery without enabling heuristic
    // automatic exploration for every run. The identity latches before publication.
    root.phase = "exploration"
    root.explorationScopeId = input.sessionID
    const scope: ScopeState = {
      sessionID: input.sessionID,
      rootSessionID: root.sessionID,
      kind: "exploration",
  ...
  if (unit.status !== "pending" && unit.status !== "queued") {
    throw new OrchestrationError("PHASE_VIOLATION", `WorkUnit ${unit.id} cannot start from ${unit.status}`)
  }
...
/** Schema visibility is a projection of live scope policy, never execution permission. */
export function filterToolsForScope<T extends { id: string }>(sessionID: string, tools: readonly T[]): T[] {
  const scope = scopes.get(sessionID)
  if (!scope || scope.kind === "root" || scope.kind === "legacy") return [...tools]
  return tools.filter(tool => {
    try {
      assertToolAllowed(sessionID, tool.id)
      return true
    } catch (error) {
      if (error instanceof OrchestrationError && error.code === "PHASE_VIOLATION") return false
      // Storage/state failures are not an empty catalog or an invitation to complete.
      throw error
    }
  })
}
...
export function assertToolAllowed(sessionID: string, toolID: string, args?: unknown) {
  ...
  currentStore().assertPersistenceHealthy(root)
  if (scope.kind === "meta_review") {
    if (scope.status !== "running" || !inspectionTools.has(toolID)) {
      throw new OrchestrationError("PHASE_VIOLATION", `Meta review scope cannot use ${toolID}`)
    }
    return
  }
  ...
  if (scope.kind === "work_unit" || scope.kind === "repair") {
    if (scope.status !== "running") throw new OrchestrationError("PHASE_VIOLATION", "A prepared worker scope is frozen until an explicit repair reopen")
...
async function targetFor(sessionID: string, workspace: string, value: string): Promise<DirectTarget | ScopedTarget> {
  const scope = scopes.get(sessionID)
  const root = rootFor(sessionID)
  if (root) currentStore().assertPersistenceHealthy(root)
...
export async function resolveRead(sessionID: string, workspace: string, value: string) {
  ...
  if (target.scope.kind === "meta_review" && target.scope.status !== "running") {
    throw new OrchestrationError("PHASE_VIOLATION", "A closed meta review cannot inspect workspace files")
  }
...
async function readCandidateFile(filepath: string, boundary: string): Promise<Buffer> {
  const root = await fs.realpath(boundary)
  const absolute = path.resolve(filepath)
  const expected = inside(root, absolute)
    ? absolute
    : path.resolve(root, path.relative(path.resolve(boundary), absolute))
  const resolved = await fs.realpath(filepath)
  if (!inside(root, expected) || platformPath(resolved) !== platformPath(expected)) {
    throw new OrchestrationError("OWNERSHIP_VIOLATION", "Candidate path crossed a link boundary: " + filepath)
  }
  const entry = await fs.lstat(filepath)
  if (!entry.isFile() || entry.nlink !== 1 || entry.size > MAX_CANDIDATE_FILE_BYTES) {
    throw new OrchestrationError("WORKSPACE_CONFLICT", "Candidate requires a single-link regular file of at most 10 MiB: " + filepath)
  }
  const handle = await fs.open(filepath, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0) | (fsConstants.O_NONBLOCK ?? 0))
  const identity = (value: typeof entry) =>
    [value.dev, value.ino, value.size, value.mtimeMs, value.ctimeMs, value.nlink].join(":")
  try {
    const before = await handle.stat()
    if (!before.isFile() || before.nlink !== 1 || identity(before) !== identity(entry)) {
      throw new OrchestrationError("WORKSPACE_CONFLICT", "Candidate changed before opening: " + filepath)
    }
    const chunks: Buffer[] = []
    let length = 0
    while (length <= MAX_CANDIDATE_FILE_BYTES) {
      const buffer = Buffer.alloc(Math.min(64 * 1024, MAX_CANDIDATE_FILE_BYTES + 1 - length))
    ...
    if (
      length > MAX_CANDIDATE_FILE_BYTES || !current.isFile() ||
      identity(before) !== identity(after) || identity(after) !== identity(current) ||
      platformPath(await fs.realpath(filepath)) !== platformPath(resolved)
    ) throw new OrchestrationError("WORKSPACE_CONFLICT", "Candidate changed while being read: " + filepath)
...
async function fileHash(filepath: string, boundary: string) {
  try {
    return hash(await readCandidateFile(filepath, boundary))
  } catch (error) {
...
export async function resolveWrite(sessionID: string, workspace: string, value: string) {
  ...
  if (target.scope.status !== "running") {
  ...
  const beforeHash = await fileHash(target.logicalPath, target.root.workspace)
...
const candidateHashPayload = (candidate: Omit<CandidateManifest, "candidateId" | "patchHash" | "overlayRoot">) => ({
  ...
  // UTF-8 lexical order matches the verifier's Unicode code-point ordering.
  files: [...candidate.files].sort((left, right) => Buffer.compare(Buffer.from(left.path), Buffer.from(right.path))),
})
...
export async function prepareCandidate(sessionID: string): Promise<CandidateManifest | undefined> {
  ...
  if (!root || !unit) return
  if (scope.status !== "running") throw new OrchestrationError("PHASE_VIOLATION", "Candidate preparation requires a running worker")
...
export async function assertCandidateIntegrity(candidateId: string): Promise<void> {
  const candidate = candidates.get(candidateId)
  if (!candidate) throw new OrchestrationError("PHASE_VIOLATION", "Unknown candidate: " + candidateId)
  if (candidate.files.length && !candidate.candidateWorkspace) {
    throw new OrchestrationError("PHASE_VIOLATION", "Candidate must retain its verification workspace until commit")
  }
  if (
    hash(canonicalJSON(candidateHashPayload(candidate))) !== candidate.patchHash ||
    candidate.scope.files.size !== candidate.files.length
  ) throw new OrchestrationError("WORKSPACE_CONFLICT", "Prepared candidate manifest or file set changed")
  try {
    for (const file of candidate.files) {
      const entry = candidate.scope.files.get(platformPath(file.path))
      if (!entry || entry.beforeHash !== file.beforeHash) {
        throw new OrchestrationError("WORKSPACE_CONFLICT", "Prepared candidate ownership changed: " + file.path)
      }
      const canonicalPath = await canonical(candidate.root.workspace, file.path)
      if (platformPath(canonicalPath) !== platformPath(file.path)) {
        throw new OrchestrationError("WORKSPACE_CONFLICT", "Candidate destination changed: " + file.path)
      }
    ...
    throw new OrchestrationError("WORKSPACE_CONFLICT", "Candidate integrity inspection failed: " + String(error))
  }
}

function withWorkspacePublication<T>(action: () => Promise<T>): Promise<T> {
  const store = currentStore()
  const operation = store.commitQueue.then(() => withWorkspaceCandidateStore(store, action))
  // A failed operation must release the queue for independent candidates.
  store.commitQueue = operation.then(() => undefined, () => undefined)
  return operation
}

export function materializeCandidate(candidateId: string): Promise<string> {
  return withWorkspacePublication(async () => {
    try {
      return await materializeCandidateLocked(candidateId)
    } catch (error) {
      if (error instanceof OrchestrationError) throw error
      const code = (error as NodeJS.ErrnoException)?.code
      const changed = code === "ENOENT" || code === "ENOTDIR" || code === "ESTALE"
      throw new OrchestrationError(
        changed ? "WORKSPACE_CONFLICT" : "CANDIDATE_MATERIALIZATION_FAILED",
        "Candidate materialization failed: " + (error instanceof Error ? error.message : String(error)),
      )
    }
  })
}

async function materializeCandidateLocked(candidateId: string): Promise<string> {
  const candidate = candidates.get(candidateId)
  if (!candidate) throw new OrchestrationError("PHASE_VIOLATION", `Unknown candidate: ${candidateId}`)
  if (candidate.candidateWorkspace) {
    await assertCandidateIntegrity(candidateId)
    return candidate.candidateWorkspace
  }
  ...
  await fs.cp(workspace, destination, {
    ...
    filter: async (source) => {
      ...
      if (!stat.isSymbolicLink()) {
        if (!stat.isFile() && !stat.isDirectory()) throw new OrchestrationError("OWNERSHIP_VIOLATION", "Candidate materialization rejects special files: " + source)
        return true
      }
      const target = await fs.realpath(source)
      if (!inside(workspace, target) || inside(target, resolved)) {
...
async function commitCandidateLocked(
  ...
  for (const entry of entries) {
    if ((await fileHash(entry.logical, root.workspace)) !== entry.beforeHash) {
      root.phase = "blocked"
      persist(root)
      throw new OrchestrationError(
        "WORKSPACE_CONFLICT",
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if ((before === null ? null : hash(before)) !== entry.beforeHash) {
        throw new OrchestrationError("WORKSPACE_CONFLICT", "Workspace changed before candidate commit: " + entry.logical)
      }
      const mode = before === null ? 0o600 : (await fs.lstat(entry.logical)).mode & 0o777
      changes.push({ path: entry.logical, before, after, mode })
    }
    await publishCandidateFiles(changes, {
      candidateId,
      journal,
      validatePath: async filepath => {
        const resolved = await canonical(root.workspace, filepath)
        if (platformPath(resolved) !== platformPath(filepath)) {
          throw new OrchestrationError("WORKSPACE_CONFLICT", "Candidate destination changed: " + filepath)
        }
      },
      readCurrent: async filepath => {
        try {
          return await readCandidateFile(filepath, root.workspace)
        } catch (error) {
          if ((error as NodeJS.ErrnoException).code === "ENOENT") return null
          throw error
        }
    ...
    unit.status = "failed"
    persist(root)
    if (error instanceof CandidateTransactionError) throw new OrchestrationError(error.code, error.message)
    throw error
  }
...
export async function finishChild(sessionID: string, success: boolean) {
  ...
  if (scope.kind === "meta_review") {
    ...
    // Review is planning audit, never worker completion or a candidate attestation.
    persist(root)
    return
  }
  if (scope.kind === "exploration") {
