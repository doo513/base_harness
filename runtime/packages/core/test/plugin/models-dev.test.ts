...
import { ModelsDevPlugin } from "@base-harness/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.BASE_HARNESS_MODELS_PATH,
          disabled: Flag.BASE_HARNESS_DISABLE_MODELS_FETCH,
        }
        Flag.BASE_HARNESS_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.BASE_HARNESS_MODELS_PATH = previous.path
          Flag.BASE_HARNESS_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
