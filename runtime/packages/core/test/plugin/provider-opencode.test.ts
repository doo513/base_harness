...
import { PluginHost } from "@base-harness/core/plugin/host"
import { OpencodePlugin } from "@base-harness/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
      ...
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("opencode")))?.methods).toEqual([
        {
          ...
          type: "oauth",
          label: "Base Harness Console account",
        },
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
      ...
      expect(attempt.url).toBe("https://base-harness.local/console/device?user_code=user&client_id=opencode-cli")
    }),
  )
  ...
  it.live("loads providers and models from the connected Base Harness server", () =>
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ BASE_HARNESS_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses BASE_HARNESS_API_KEY as credentials", () =>
    withEnv({ BASE_HARNESS_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ BASE_HARNESS_API_KEY: undefined, CUSTOM_BASE_HARNESS_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_BASE_HARNESS_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
