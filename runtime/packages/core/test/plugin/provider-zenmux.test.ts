...
import { ProviderPlugins } from "@base-harness/core/plugin/provider"
import { ZenmuxPlugin } from "@base-harness/core/plugin/provider/zenmux"
...
describe("ZenmuxPlugin", () => {
  ...
  it.effect("applies the exact legacy Zenmux headers", () =>
    Effect.gen(function* () {
      ...
      expect(result.request.headers).toEqual({ "HTTP-Referer": "https://base-harness.local/", "X-Title": "opencode" })
