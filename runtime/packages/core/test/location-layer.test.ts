...
import { define } from "@base-harness/plugin/v2/effect"
...
import { LocationServiceMap } from "@base-harness/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "base-harness.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "base-harness.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
