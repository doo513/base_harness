...
import { ConfigProvider } from "@base-harness/core/config/provider"
...
import { ConfigMigrateV1 } from "@base-harness/core/v1/config/migrate"
...
import { Policy } from "@base-harness/core/policy"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "base-harness.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "base-harness.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "base-harness.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "base-harness.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
  ...
  it.live("accepts $schema metadata without writing it into config files", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "base-harness.json")
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "base-harness.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "base-harness.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "base-harness.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .base-harness configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".base-harness"), { recursive: true })
            await fs.mkdir(path.join(directory, ".base-harness"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "base-harness.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "base-harness.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "base-harness.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "base-harness.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "base-harness.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".base-harness", "base-harness.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".base-harness", "base-harness.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".base-harness")),
              AbsolutePath.make(path.join(directory, ".base-harness")),
            ])
