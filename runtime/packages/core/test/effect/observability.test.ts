...
const opencodeClient = process.env.BASE_HARNESS_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.BASE_HARNESS_CLIENT
  else process.env.BASE_HARNESS_CLIENT = opencodeClient
})
...
describe("resource", () => {
  ...
  test("keeps built-in attributes when env values conflict", () => {
    process.env.BASE_HARNESS_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
