...
export class Service extends Context.Service<Service, Interface>()("@base-harness/ProjectV2") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "base-harness")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "base-harness"), input.id).pipe(Effect.ignore)
    })
