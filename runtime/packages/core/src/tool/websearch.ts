...
import { ToolFailure } from "@base-harness/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@base-harness/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.BASE_HARNESS_WEBSEARCH_PROVIDER === "exa" || process.env.BASE_HARNESS_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.BASE_HARNESS_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("BASE_HARNESS_EXPERIMENTAL") || truthy("BASE_HARNESS_ENABLE_EXA") || truthy("BASE_HARNESS_EXPERIMENTAL_EXA"),
    enableParallel: truthy("BASE_HARNESS_ENABLE_PARALLEL") || truthy("BASE_HARNESS_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `base-harness/${InstallationVersion}`,
