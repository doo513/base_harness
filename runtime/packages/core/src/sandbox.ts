...
for source in /bin /usr /lib /lib64 /etc /dev /sys; do
  ...
  mount -o remount,ro,bind "$root$source" 2>/dev/null || true
done
...
async function wslRun(request: SandboxRequest, policy: SandboxPolicy, root: string, inputBytes: number) {
  ...
  const probe = await checked([...prefix, "/bin/sh", "-lc", "command -v unshare prlimit timeout setsid mount chroot >/dev/null && uname -r"])
