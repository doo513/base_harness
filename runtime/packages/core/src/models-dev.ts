...
import { ModelsDev } from "@base-harness/schema/models-dev"
...
const USER_AGENT = `base-harness/${InstallationChannel}/${InstallationVersion}/${Flag.BASE_HARNESS_CLIENT}`
...
declare const BASE_HARNESS_MODELS_DEV: Record<string, Provider> | undefined
...
export class Service extends Context.Service<Service, Interface>()("@base-harness/ModelsDev") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadFromDisk = fs.readJson(Flag.BASE_HARNESS_MODELS_PATH ?? filepath).pipe(
      Effect.catch((error) => {
        if (
    ...
    if (!Flag.BASE_HARNESS_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
