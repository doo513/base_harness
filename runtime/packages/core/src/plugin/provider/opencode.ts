...
import type { IntegrationOAuthMethodRegistration } from "@base-harness/plugin/v2/effect/integration"
import { define } from "@base-harness/plugin/v2/effect/plugin"
import type { CredentialValue } from "@base-harness/sdk/v2/types"
...
const defaultServer = "https://base-harness.local/console"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Base Harness provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      const hasKey = Boolean(process.env.BASE_HARNESS_API_KEY || connected || item.provider.request.body.apiKey)
