...
import { PromptInput } from "@base-harness/schema/prompt-input"
import { Session } from "@base-harness/schema/session"
import { Project } from "@base-harness/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@base-harness/schema/schema"
...
import { Revert } from "@base-harness/schema/revert"
import { SessionEvent } from "@base-harness/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Base Harness process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Base Harness process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
