...
This is the living design and status document for `@base-harness/codemode` and its existing V2 Base Harness adapter.
...
### V2 Base Harness adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/base-harness/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
