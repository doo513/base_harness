/**
 * Autonomous execution wire contracts. Objects are data, never capabilities.
 * Host/Coordinator authenticate producers and Kernel validates admission.
 * Legacy GoalContract contracts remain unchanged during migration.
 */

export type Json = null | boolean | number | string | Json[] | { [key: string]: Json }

import type { ReadonlyValue } from "./domain"
...
/** App/model response before the Host allocates the immutable report artifact. */
export interface AutonomousFinalResponse {
  kind: "final"
  text: string
...
export interface AutonomousPreparationInput {
...
/** Candidate context, not accepted authority or a mandatory solution plan. */
export type AutonomousPreparationResult =
  | { status: "proceed"; context: Json }
  | { status: "needs_input"; reason: "information" | "authority"; questions: string[] }
  | { status: "invalid"; code: string }

/** Pure Domain callbacks. Host validates every return and owns all I/O and state. */
export interface AutonomousDomainModule {
  readonly id: string
  readonly revision: string
  readonly domainId: string
  prepare(input: ReadonlyValue<AutonomousPreparationInput>): AutonomousPreparationResult
  normalizeDecision(input: {
    response: unknown
    /** Only supplied after Host has actually stored a plain-text final artifact. */
    textReport?: ReadonlyValue<SubjectRef & { kind: "report" }>
  }): unknown
}
