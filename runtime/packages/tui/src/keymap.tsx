...
export const BASE_HARNESS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const BASE_HARNESS_MODE_KEY = "base-harness.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(BASE_HARNESS_MODE_KEY, BASE_HARNESS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(BASE_HARNESS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(BASE_HARNESS_MODE_KEY, stack.at(-1)?.mode ?? BASE_HARNESS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? BASE_HARNESS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(BASE_HARNESS_MODE_KEY, undefined)
