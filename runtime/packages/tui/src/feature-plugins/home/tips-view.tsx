import type { TuiPluginApi } from "@base-harness/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("base-harness.status"),
...
const TIPS: Tip[] = [
  ...
  "Configure the harness in {highlight}base-harness.jsonc{/highlight} and TUI preferences in {highlight}tui.json{/highlight}",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.base-harness/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.base-harness/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.base-harness/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.base-harness/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Base Harness from reading sensitive files",
  "Use {highlight}base-harness run{/highlight} for non-interactive scripting",
  "Use {highlight}base-harness --continue{/highlight} to resume the last session",
  "Use {highlight}base-harness run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}base-harness serve{/highlight} for headless API access to Base Harness",
  "Use {highlight}base-harness run --attach{/highlight} to connect to a running server",
  "Run {highlight}base-harness auth list{/highlight} to see all configured providers",
  "Run {highlight}base-harness agent create{/highlight} for guided agent creation",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.base-harness/themes/{/highlight} directory",
  ...
  "Run {highlight}base-harness debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} with Base Harness Zen for curated, tested models",
