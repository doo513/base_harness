...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".base-harness/glossary/zh-cn.md"
  if (locale === "zht") return ".base-harness/glossary/zh-tw.md"
  return `.base-harness/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Base Harness did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Base Harness returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Base Harness session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Base Harness runs for 'all' (default: 4)
      --model <provider/id>  Base Harness model (default: opencode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Base Harness
...
`)
  ...
  if (failed.length) console.error(`\nBase Harness failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.BASE_HARNESS_DISABLE_PROJECT_CONFIG = "1"
  env.BASE_HARNESS_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["base-harness", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.BASE_HARNESS_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["base-harness", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.BASE_HARNESS_CONFIG
  delete env.BASE_HARNESS_CONFIG_DIR
  delete env.BASE_HARNESS_CONFIG_CONTENT
  delete env.BASE_HARNESS_PERMISSION
  delete env.BASE_HARNESS_AUTO_SHARE
  return env
}
