...
class MCPStdioClient:
    """Minimal MCP stdio client supporting modern 2026 and legacy 2025 eras.

    The client implements only the host surfaces needed by this harness phase:
    discovery/initialization, tools/list, and tools/call. Server-initiated
    requests, subscriptions, tasks, prompts, resources, roots, sampling, and
    logging are intentionally outside this minimal contract.
    """
    ...
    def _write(self, message: dict[str, Any]) -> None:
        ...
        if process is None or process.stdin is None or process.poll() is not None:
            raise MCPError("MCP stdio server is not running")
        try:
    ...
    def list_tools(self, *, max_pages: int = 20, max_tools: int = 512) -> list[MCPToolDefinition]:
        self.connect()
...
class MCPGateway:
    ...
    def _tool_policy(config: MCPServerConfig, tool_name: str) -> tuple[SideEffect, str, bool]:
        ...
        if not isinstance(policies, dict):
            raise ConfigError(f"MCP server {config.name} options.tool_policies must be an object")
    ...
    def _client(self, config: MCPServerConfig) -> MCPStdioClient:
        ...
        if config.transport != "stdio":
            raise MCPError(
                f"MCP transport {config.transport!r} is declared but not implemented in mcp-gateway-v1"
            )
        ...
        for server in self.servers:
            ...
            for definition in client.list_tools():
                ...
                spec = ToolSpec(
                    ...
                    # Full MCP JSON Schema 2020-12 is preserved for the model but
                    # not misrepresented as validated by our local subset.
                    input_schema=None,
                    output_schema=None,
                )
