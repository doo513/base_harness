...
class Profile(DomainProfile):
    ...
    def __init__(self, workspace: Path, tools=None):
        self.workspace = workspace; self._tools = dict(tools or {})
    def default_goal(self): return GoalContract(goal="stage6 boundary probe", acceptance=["done"])
    def tools(self): return dict(self._tools)
    def completion_oracle(self):
        return PredicateCompletionOracle(lambda **_: CompletionResult(True, "done"), name="stage6-boundary-oracle")
...
def run_probe() -> dict:
    with tempfile.TemporaryDirectory(prefix="stage6-boundary-") as td:
        ...
        ws = root / "facts-ws"; ws.mkdir(); r = runtime(root, "facts", Profile(ws), ScriptedController([]))
        r.state.commit_verified(Claim("answer", "alpha beta", status=ClaimStatus.VERIFIED,
        ...
        r.state.commit_verified(Claim("answer", "alpha   beta", status=ClaimStatus.VERIFIED,
                                      authority=Authority.ENVIRONMENT, evidence_refs=["artifact://three"]))
        h3 = r._progress_facts_hash()
        outcomes["fact_metadata_churn_not_progress_whitespace_preserved"] = {
            "passed": h1 == h2 and h3 != h1,
            "metadata_hash_equal": h1 == h2,
            "whitespace_value_distinct": h3 != h1,
        }

        ws2 = root / "global-ws"; ws2.mkdir()
        decisions = [Decision("propose", {"key": k, "value": k}) for k in "abcdef"] + [Decision("complete", {"reason": "done"})]
        r2 = runtime(root, "global", Profile(ws2), ScriptedController(decisions),
                     ProgressPolicy(family_repeat_limit=10, no_progress_streak_limit=2,
                                    max_strategy_generations_without_progress=5))
        s2 = r2.run()
        failures = [f for f in s2.failures if f["kind"] == FailureKind.NO_PROGRESS.value
        ...
        outcomes["global_repeat_identity_stable"] = {
            ...
            "repeat_counts": [f["repeat_count"] for f in failures], "actions": actions,
        }

        ws3 = root / "tamper-ws"; ws3.mkdir()
        tool = ToolSpec(name="observe", description="read", handler=lambda: {"stable": True},
                        side_effect=SideEffect.NONE, idempotent=True,
                        provenance={"revision": "stage6-boundary"})
        controller = ScriptedController([Decision("tool", {"tool": "observe", "args": {}}),
        ...
        r3.step_once()
        terminal_scheduled = controller.index == 1 and r3.state.pending_recovery is not None \
        ...
        outcomes["historical_tamper_terminalized_before_actor"] = {
            "passed": terminal_scheduled and r3.halted and r3.state.recovery_history[-1].action == RecoveryAction.CHECKPOINT_STOP,
            "actor_cursor": controller.index, "terminal_scheduled": terminal_scheduled, "halted": r3.halted,
        }

        ws4 = root / "canon-ws"; ws4.mkdir(); r4 = runtime(root, "canon", Profile(ws4), ScriptedController([]))
        a = r4.artifacts.put_json("a.json", {"ok": True, "output": {"a": "x y", "b": 2}, "error": None})
        b = r4.artifacts.put_json("b.json", {"error": None, "output": {"b": 2, "a": "x y"}, "ok": True})
        c = r4.artifacts.put_json("c.json", {"ok": True, "output": {"a": "x   y", "b": 2}, "error": None})
        fa, fb, fc = map(r4._verified_observation_fingerprint, (a, b, c))
        outcomes["evidence_key_order_canonical_whitespace_sensitive"] = {
            "passed": fa == fb and fa != fc,
            "key_order_equal": fa == fb,
            "whitespace_distinct": fa != fc,
        }

        result = {"stage": "06", "probe": "progress-boundary-rc3", "outcomes": outcomes}
        result["summary"] = {
            ...
            "fact_metadata_false_progress": 0 if outcomes["fact_metadata_churn_not_progress_whitespace_preserved"]["metadata_hash_equal"] else 1,
            "whitespace_false_no_progress": 0 if outcomes["fact_metadata_churn_not_progress_whitespace_preserved"]["whitespace_value_distinct"] else 1,
        }
        return result


def main() -> int:
    result = run_probe(); print(json.dumps(result, indent=2, sort_keys=True))
